...
begin
  ...
  insert into public.agreement_versions (agreement_id, version, snapshot, content_hash)
  values (p_agreement_id, v_next, p_snapshot, v_hash);

  update public.agreements
  set version = v_next, updated_at = now()
...
begin
  ...
  if not found then raise exception 'Agreement not found'; end if;
  if v_agreement.status in ('client_signed','executed','superseded') then
    raise exception 'Agreement cannot be sent in its current state';
  end if;
  update public.agreements
