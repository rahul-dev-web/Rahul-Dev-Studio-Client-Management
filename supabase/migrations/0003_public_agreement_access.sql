-- Secure private agreement links, client correction requests and public client signing.
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare aid uuid; cid uuid; acode text; aversion integer; sigid uuid; existing integer;
begin
 ...
 select a.id,a.client_id,a.agreement_code,a.version into aid,cid,acode,aversion from public.agreements a where a.access_token_hash=p_token_hash and a.status in ('sent','under_review');
 if aid is null then raise exception 'This agreement is not available for signing'; end if;
 if cid is null then raise exception 'Client account is not yet associated with this agreement'; end if;
 select count(*) into existing from public.signatures where agreement_id=aid and signer_role='client';
 if existing>0 then raise exception 'This agreement has already been signed by the client'; end if;
 insert into public.signatures(agreement_id,signer_id,signer_role,signer_name,signature_data) values(aid,cid,'client',trim(p_signer_name),p_signature_data) returning id into sigid;
