...
begin
  if not public.is_developer() then
    raise exception 'Only developers can issue agreement access links';
  end if;
  update public.agreements
  set access_token_hash = p_token_hash,
      status = case when status = 'draft' then 'sent'::public.agreement_status else status end,
...
begin
  select jsonb_build_object(
    'agreement', jsonb_build_object(
      ...
      'developer_signed_at', a.developer_signed_at
    ),
    'deal', jsonb_build_object(
      ...
      'support_days', d.support_days
    ),
    'version_snapshot', av.snapshot,
    'scope', coalesce((select jsonb_agg(jsonb_build_object('title', ds.title, 'item_order', ds.item_order) order by ds.item_order) from public.deal_scope ds where ds.deal_id = d.id), '[]'::jsonb),
    'deliverables', coalesce((select jsonb_agg(jsonb_build_object('title', dd.title, 'item_order', dd.item_order) order by dd.item_order) from public.deal_deliverables dd where dd.deal_id = d.id), '[]'::jsonb)
  ) into result
  from public.agreements a
  join public.deals d on d.id = a.deal_id
  left join lateral (select snapshot from public.agreement_versions x where x.agreement_id = a.id and x.version = a.version limit 1) av on true
  where a.access_token_hash = p_token_hash;

  if result is null then raise exception 'Invalid or expired agreement link'; end if;
  return result;
end;
$$;
