...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  // Executed agreements are intentionally immutable. Portal identity is linked through
  // the mutable deal/project/profile records instead of updating agreements.client_id.
  const password=tempPassword(); let clientUserId:string|undefined=deal.client_id||agreement.client_id||undefined;
  ...
  const code=currentProfile?.client_code||clientCode();
  const {error:profileError}=await admin.from("profiles").upsert({id:clientUserId,role:"client",full_name:deal.client_name,client_code:code,must_change_password:true,updated_at:new Date().toISOString()},{onConflict:"id"});if(profileError) throw new Error(`Unable to save client profile: ${profileError.message}`);
