...
function generateClientCode() {
  return `RDSC-${randomBytes(4).toString("hex").toUpperCase()}`;
}
...
export async function POST(request: Request) {
  try {
    ...
    if (profileError || profile?.role !== "developer") {
      return NextResponse.json({ error: "Developer authorization required." }, { status: 403 });
    }

    const { data: agreement, error: agreementError } = await supabase
      .from("agreements")
      .select("id, status, client_id, deal_id, project_id")
      .eq("id", agreementId)
      .single();
    ...
    if (!serviceRoleKey) return NextResponse.json({ error: "SUPABASE_SERVICE_ROLE_KEY is not configured on the server." }, { status: 500 });
    ...
    if (clientUserId) {
      ...
      if (existing) {
        ...
        const { data: existingProfile, error: existingProfileError } = await supabase.from("profiles").select("role").eq("id", clientUserId).single();
        if (!existingProfileError && existingProfile?.role === "developer") return NextResponse.json({ error: "This email belongs to a developer account and cannot be provisioned as a client." }, { status: 409 });
        const { error: updateError } = await admin.auth.admin.updateUserById(clientUserId, { password: temporaryPassword, email_confirm: true });
        if (updateError) throw updateError;
      } else {
        const { data: created, error: createError } = await admin.auth.admin.createUser({
          ...
          email_confirm: true,
          user_metadata: { full_name: deal.client_name, organization: deal.organization, role: "client" },
        });
    ...
    if (agreement.project_id) {
      const { error: projectUpdateError } = await admin.from("projects").update({ client_id: clientUserId }).eq("id", agreement.project_id);
    ...
    return NextResponse.json({
      ...
      message: "Client portal credentials generated. Show the temporary password to the client once and require a password change on first login.",
    });
  } catch (error) {
