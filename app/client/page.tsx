...
import { ArrowUpRight, Check, Clock3, FileSignature, LogOut, MessageSquare, ShieldCheck } from "lucide-react";
...
export default function ClientPortal() {
  ...
  useEffect(()=>{(async()=>{
    ...
    // Production projects schema uses expected_delivery_date (not expected_date).
    // The portal is linked directly through projects.client_id after credentials are provisioned.
  ...
  return <main className="client-shell">
    ...
    <div className="client-main">
      ...
      <aside className="panel"><div className="eyebrow">DOCUMENTS & SUPPORT</div><div className="panel-title"><ShieldCheck size={16}/> Agreement</div><p className="page-copy">{agreement?.agreement_code||"Agreement pending"}</p><div className="portal-card-row"><span>Status</span><strong>{label(agreement?.status)}</strong></div><a className="btn primary" href={agreement?`/agreements/${agreement.id}`:"#"}><FileSignature size={14}/> View agreement <ArrowUpRight size={13}/></a><div className="action-divider"/><div className="panel-title"><MessageSquare size={16}/> Need a change?</div><p className="page-copy">Corrections can be requested before signing. Signed agreements require a formal amendment.</p>{agreement&&["sent","under_review"].includes(agreement.status)&&<a className="btn" href={`/agreements/${agreement.id}`}><MessageSquare size={14}/> Request correction</a>}</aside></div>
